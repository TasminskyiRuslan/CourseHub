    ...
    */
    describe('validation', function () {
        it('fails if the required fields are missing', function () {
            postJson(route('account.register'), [])
                ->assertUnprocessable()
        ...
        it('fails if the email is already taken', function () {
            ...
            postJson(route('account.register'), $data)
                ->assertUnprocessable()
                ->assertJsonValidationErrors(['email']);
        });

        it('fails if the email format is invalid', function () {
            postJson(route('account.register'), registrationPayload(['email' => 'invalid-email']))
                ->assertUnprocessable()
                ->assertJsonValidationErrors(['email']);
        });

        it('fails if the password is too short', function () {
            postJson(route('account.register'), registrationPayload([
                'password' => '123',
        ...
        it('fails if the role is invalid', function () {
            postJson(route('account.register'), registrationPayload(['role' => 'invalid-role']))
                ->assertUnprocessable()
    ...
    */
    describe('success', function () {
        ...
        it('sets a short token expiration by default', function () {
            ...
            $response = postJson(route('account.register'), registrationPayload())
                ->assertCreated();
