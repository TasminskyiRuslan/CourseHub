...
describe('AccountImageController -> update', function () {
    ...
    */
    describe('validation', function () {
        it('fails if the required fields are missing', function () {
            ...
            postJson(route('auth.account.image.update'), ['_method' => 'PUT'])
                ->assertUnprocessable()
        ...
        it('fails if the image is not a file', function () {
            ...
            postJson(route('auth.account.image.update'), imagePayload([
                'image' => 'not-a-file',
            ]))
                ->assertUnprocessable()
                ->assertJsonValidationErrors(['image']);
    ...
    */
    describe('permissions', function () {
        ...
        it('allows authenticated users with any role to update their own account image', function ($user) {
            ...
            postJson(route('auth.account.image.update'), $data)
                ->assertOk()
                ->assertJsonStructure(['data' => userJsonStructure()]);
